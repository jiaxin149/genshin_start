...
var BGM = new Audio("./music/genshin.mp3"); //BGM
...
const LANGUAGES = {
    ...
    "cn": {
        audioList: [
            "audio/CN_voice/原神.mp3",
            "audio/CN_voice/神原.mp3",
            "audio/CN_voice/启动.mp3",
            "audio/CN_voice/动启.mp3"
        //0-10：默认模式 mode1
        ...
        texts: {
            "page-title": "原神，启动！",
            "doc-title": "原神，启动！",
            "page-descriptions": "什么<br>原来你也玩原神？！",
            "counter-descriptions": ["原神启动次数"],
            "counter-unit": ["次", "次"],
            "counter-button": ["启动！", "启动？"],
...
function refreshLanguageSelector(selectedLanguage) {
    switch (selectedLanguage) {
        case "cn":
            document.getElementById("lan1")
                .innerHTML = "🌏中文";
            document.getElementById("lan2")
                .innerHTML = "日文";
            document.getElementById("lan3")
                .innerHTML = "英文";
            break;
        case "ja":
            document.getElementById("lan1")
...
function refreshModeSelector(selectedLanguage) {
    switch (selectedLanguage) {
        ...
        case "en":
            document.getElementById("mode1")
                .innerHTML = "DefaultMode";
            document.getElementById("mode2")
                .innerHTML = "Just Dadada";
            document.getElementById("mode3")
                .innerHTML = "Easter egg";
            break;
        default:
            break;
    }
...
function refreshDynamicTexts() {
    ...
    Object.entries(localTexts)
        .forEach(([textId, value]) => {
        if (value instanceof Array) if (document.getElementById(textId) != undefined) document.getElementById(textId)
            .innerHTML = randomChoice(value);
    });
};
...
function multiLangMutation() {
    ...
    Object.entries(localTexts)
        .forEach(([textId, value]) => { //将各个DOM元素的值变为文本值
        if (!(value instanceof Array)) if (document.getElementById(textId) != undefined) document.getElementById(textId)
            .innerHTML = value;
    });
    refreshDynamicTexts() //刷新动态文本内容（将新的DOM值渲染到前端）
    document.getElementById("klee-card")
        .src = curLang.cardImage; //动态获取klee-card在当前语言文本环境下的Dom元素并更新

    var selectedLanguage = document.getElementById("language-selector")
        .value;
...
function tryCachedObject(origUrl) {
    ...
    if (cachedObjects[origUrl]) {
        ...
        fetch(origUrl)
            ...
            console.error(`Error caching object from $ {
                origUrl
            }: $ {
                error
            }`);
...
function cacheAllResources() {
    // 缓存图片资源
    for (let i = 1; i < 6; i++) {
        const imageUrl = `img/animation/${i}.png`;
...
function getRandomAudioUrl(curMode) {
    ...
    //根据模式产生符合模式的音频下标
    if (curMode == "mode1") {
        randomIndex = Math.floor(Math.random() * 4); //默认语音的文件是：文件下标0-10.这儿的数字就写8.
    }
    if (curMode == "mode3") {
        randomIndex = Math.floor(Math.random() * 1); //文件下标0-16.这儿的数字就写14. 【这儿写彩蛋对应的audioList下标】
    }
...
function animateKlee() {
    ...
    const random = Math.floor(Math.random() * 10) + 1; //1-5，可莉的GIF
    const elem = document.createElement("img");
    elem.src = tryCachedObject(`img/animation/${random}.png`);
...
document.addEventListener("DOMContentLoaded", function() {
    $.ajax({
        ...
        success: function(response) {
            ...
            globalCountElement.textContent = (response.globalcountsFromBackend)
...
setInterval(function() {
    $.ajax({
        ...
        success: function(response) {
            ...
            globalCountElement.textContent = (response.globalcountsFromBackend)
                .toLocaleString('en-US') //更新全球计数器的值
